...
上述指令中，有些指令需要以其他指令先执行为前提。比如部分类成员访问（如`getfield`和`putfield`）和方法调用（如`invokeinterface`和`invokevirtual`）需要先有对象实例，那么一定会先执行对象创建指令（如`new`），而对象创建、类型判断需要先执行ldc把类型的符号引用或描述符从常量池加载到操作数栈上。基于这类前提条件，上述各指令未能通过测试用力全部检验依赖带来的影响效果。
