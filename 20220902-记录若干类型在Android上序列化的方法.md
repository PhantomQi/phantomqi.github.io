...
需要注意的是：上述两个方法的读和写方法最好配套使用，而不要交叉使用，以免发生意料之外的结果。具体情况需要结合`Parcel`和具体`Parcelable`实现类的源码分析。
...
@Test
public void 验证marshall和unmarshall的实现() {
    通过bytes验证Bundle的双向转换(true, TransferUtil::marshallToBytes, bytes -> {
        return unmarshallToParcelable(bytes, Bundle.CREATOR);
    });
}
```
